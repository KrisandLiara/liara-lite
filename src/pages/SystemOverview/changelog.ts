...
export interface Change {
  type?: 'feature' | 'fix' | 'refactor' | 'docs' | 'perf' | 'improvement';
  title?: string;
  description?: string;
  category?: string;
  summary?: string;
...
export const changelogData: ChangelogEntry[] = [
  {
    ...
    changes: [
      {
        type: 'improvement',
        title: 'Maps Lab — right-side tuning sidebar',
        description:
          'Replaced the in-content tuning panel with a container-anchored right rail + sidebar (no layout push), with staged “robot” open animation (width first, then full height). Sidebar stays visible while scrolling.'
      },
      {
        type: 'improvement',
        title: 'Per-viz tuning + GLOBAL controls',
        description:
          'Tuning panel now shows controls based on the active visualization, with GLOBAL knobs clearly labeled. Added 5 advanced tuning options per viz (Voronoi / Pack / Force / Pixi).'
      },
      {
        type: 'perf',
        title: 'Snappier tuning UX (commit-on-release sliders)',
        description:
          'Sliders update draft values while dragging and only apply to the viz on release; added numeric inputs for precise edits and compact slider styling for denser control layouts.'
      }
    ]
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        title: 'Maps Lab (TEST) — “Memory Geography” prototypes',
        description:
          'Added a new Maps Lab route with shared token indexing (tags + NER entities) and multiple prototype visualizations for exploring “tag countries”.'
      },
      {
        type: 'improvement',
        title: 'Voronoi “Countries” map interactions',
        description:
          'Added zoom/pan + label LOD (fade-in as you zoom) and stabilized layout so clicking a cell selects without reshuffling the map.'
      },
      {
        type: 'improvement',
        title: 'Inline tuning panel UX',
        description:
          'Moved tuning into the Controls panel and changed it from an overlay drawer to an inline, slide-open panel aligned with the main layout.'
      }
    ]
  },
  {
